    ...
    if( !empty( $arr_table ) ){
		...
		function copy_data_from_user(el) {
			...
			if (jQuery(el).prop("checked")) {
				...
				jQuery('.course-fields-wrapper').each(function () {
...
function naf_course_add_course_to_user( $order_id ){
	...
	if( get_post_meta( $order_id, 'addd_course_to_user', true ) ){
		error_log("Already processed order_id: " . $order_id);
		return;
	}
		...
		if( $data ){
			...
			foreach ( $data as $key => $item ) {
				...
				foreach ( $item as $in_key => $value ) {
					...
					error_log("Processing user with email: " . $user_email);

					if( email_exists( $user_email ) ){
						error_log("Email exists, getting user by email.");
                        ...
                        $id = wp_insert_user([
                            ...
                            'user_pass' => $random_password,   // הגדרת סיסמה אקראית
						...
						if (is_wp_error($id)) {
							...
							continue; // דלג על נרשם זה
						}

						update_user_meta( $id , 'billing_phone' , $value->tel );
						update_user_meta( $id , 'billing_email' , $user_email );
					...
					if( !is_array($courses) || !is_object($courses) ){
						$courses = [];
						error_log("getCoursePerUser returned a non-countable value, defaulting to empty array.");
					}

					if( count( (array)$courses ) == 0 ){
						error_log("User is not in course, adding them now.");
			...
			error_log("No register_users_data found in order.");
		}
...
add_action('woocommerce_order_status_processing', 'naf_course_add_course_to_user', 10, 1);
add_action('woocommerce_order_status_completed', 'naf_course_add_course_to_user', 10, 1);

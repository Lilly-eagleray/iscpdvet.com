...
if ($post->post_type != 'shop_order'){
   ...
   if($order->get_status() == "completed"){
	...
	$user_id = get_user_by( 'email', $order_billing_email)->ID;
    ...
    if( is_null( $checkIfExsist ) || ( is_array($checkIfExsist) && count( $checkIfExsist ) == 0 ) ){
 ...
 // mail("liranhecht@gmail.com","sdfdsf","");
...
function sm_woo_custom_checkout_button_text() {
  ...
  ?>
       <a href="<?php echo $checkout_url; ?>" class="checkout-button button alt wc-forward"><?php  _e( 'Registration and payment', 'ultimate-course' ); ?></a> 
  <?php
} 
...
function drerez_display_order_data_in_admin( $order ){
	$test_order = new WC_Order($order);
	$order_key = $test_order->order_key;
	$url = "<a target='_blank'	href='https://iscpdvet.com/checkout/?key=" . $order_key . "&order=".$test_order->id."'>עבור לדף התשלום</a>";
	echo $url;
}
